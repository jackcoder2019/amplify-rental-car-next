...
import { listRentalCars } from "../graphql/queries";
...
export default function NewCardCollection(props) {
    ...
    while ((newCache.length < cacheUntil || !isPaginated) && newNext != null) {
      ...
      const result = (
        await client.graphql({
          query: listRentalCars.replaceAll("__typename", ""),
          variables,
        })
      ).data.listRentalCars;
  ...
  return (
    <div>
      <Collection
        ...
        {...getOverrideProps(overrides, "NewCardCollection")}
        {...rest}
      >
        {(item, index) => {
          if (loading) {
